...
struct parses_nested_function_calls_with_different_priorities_suite_fixture {
    ...
    void setup() {
        ...
        add(context, "bar", function_t{ function_type::infix, 2 }, FUNCTION_ID_UNKNOWN);
        add(context, "baz", function_t{ function_type::prefix, 2 }, FUNCTION_ID_UNKNOWN);
        add(context, "foo", function_t{ function_type::prefix, 3 }, FUNCTION_ID_UNKNOWN);
    }
};
...
struct parses_positional_arguments_suite_fixture {
	context_t context;
...
BOOST_FIXTURE_TEST_SUITE(parses_positional_arguments_suite,
						 parses_positional_arguments_suite_fixture)
	...
	BOOST_AUTO_TEST_CASE(case3) {
		...
		parse(tokens, tree, context);
		BOOST_CHECK(tree.src == (tree_src_t{
				{CALL_TREE_SRC_NIL, 1}, {CALL_TREE_SRC_NIL}, {0}
		}));
	}

BOOST_AUTO_TEST_SUITE_END()

struct parses_postinfix_positional_arguments_suite_fixture {
    context_t context;

    void setup() {
        initialize(context);

        add(context, "(", function_t{ function_type::postinfix }, FUNCTION_ID_UNKNOWN, ")");
        add(context, ",", function_t{ function_type::infix, 2 }, FUNCTION_ID_UNKNOWN);
    }
};

BOOST_FIXTURE_TEST_SUITE(parses_postinfix_positional_arguments_suite,
                         parses_postinfix_positional_arguments_suite_fixture)
	...
	BOOST_AUTO_TEST_CASE(case4) {
		...
		parse(tokens, tree, context);
		BOOST_CHECK(tree.src == (tree_src_t{
				{CALL_TREE_SRC_NIL, 1, 3}, {CALL_TREE_SRC_NIL, 2}, {}, {}, {0}
		}));
	}
	...
	BOOST_AUTO_TEST_CASE(case5) {
		...
		parse(tokens, tree, context);
		BOOST_CHECK(tree.src == (tree_src_t{
				{CALL_TREE_SRC_NIL, 1, 4}, {CALL_TREE_SRC_NIL, 2, 3}, {}, {}, {}, {0}
		}));
	}
...
struct parses_default_brackets_suite_fixture {
    ...
    void setup() {
        ...
        add(context, ";", function_t{ function_type::postfix, 1 }, FUNCTION_ID_UNKNOWN);
        add(context, "{", function_t{ function_type::postprefix, 0 }, FUNCTION_ID_UNKNOWN, "}");
        add(context, "(", function_t{ function_type::postprefix, 0 }, FUNCTION_ID_UNKNOWN, ")");
    }
};
...
BOOST_FIXTURE_TEST_SUITE(parses_default_brackets_suite,
    ...
    BOOST_AUTO_TEST_CASE(case3) {
        ...
        std::vector<token_t> tokens = {
                token_t{ token_type::atom, "(", 0, 0 },
                token_t{ token_type::atom, "1", 0, 0 },
                token_t{ token_type::atom, ",", 0, 0 },
                token_t{ token_type::atom, "2", 0, 0 },
                token_t{ token_type::atom, ")", 0, 0 },
        };
        parse(tokens, tree, context);
        BOOST_CHECK(tree.src == (tree_src_t{
